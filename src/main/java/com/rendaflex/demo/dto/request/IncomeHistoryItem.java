...
public record IncomeHistoryItem(
        ...
        @Pattern(regexp = "\\d{4}-(0[1-9]|1[0-2])", message = "O mês deve seguir o formato YYYY-MM.")
        String month,

        @NotNull(message = "O valor da renda é obrigatório.")
        @DecimalMin(value = "0.0", message = "O valor da renda deve ser maior ou igual a zero.")
        BigDecimal amount
) {
}
