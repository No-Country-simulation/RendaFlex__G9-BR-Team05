...
@Component
public class TransactionClassificationClient {

    static final String TRANSACTION_CLASSIFICATION_PATH =
            "/internal/v1/transactions/classify";

    private final RestClient restClient;
    private final int maximumRetries;

    public TransactionClassificationClient(
    ...
    public InternalTransactionClassificationResponse classify(
            InternalTransactionClassificationRequest request
    ) {
        Objects.requireNonNull(
                request,
                "InternalTransactionClassificationRequest must not be null."
        );
        ...
        while (true) {
            try {
                InternalTransactionClassificationResponse response =
                        restClient.post()
                                .uri(TRANSACTION_CLASSIFICATION_PATH)
                                .contentType(MediaType.APPLICATION_JSON)
                                .accept(MediaType.APPLICATION_JSON)
                                .body(request)
                                .retrieve()
                                .body(InternalTransactionClassificationResponse.class);
    ...
    private RuntimeException mapHttpFailure(int status) {
        return switch (status) {
            ...
            case 422 -> new BusinessRuleException(
                    "Não foi possível concluir a classificação de transações com os dados informados."
            );
    ...
    private ModelServiceException unavailable() {
        return new ModelServiceException(
                ...
                "O serviço de classificação de transações está temporariamente indisponível."
        );
    }
    ...
    private ModelServiceException timeout() {
        return new ModelServiceException(
                ...
                "O serviço de classificação de transações demorou mais do que o esperado para responder."
        );
    }
}
