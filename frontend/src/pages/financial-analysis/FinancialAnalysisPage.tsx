...
import { analyzeFinancialSituation } from '../../services/mocks/financialAnalysisMockService'
...
export function FinancialAnalysisPage() {
  const nextIncomeId = useRef(4); const nextTransactionId = useRef(2); const navigate = useNavigate(); const { setResult, clearResult } = useFinancialAnalysis()
  const [formState, setFormState] = useState<FinancialAnalysisFormState>({ incomeHistory: createInitialIncomeHistory(), savingFrequency: '', transactions: [{ id: '1', description: '', amount: '', date: '', type: '' }] })
  const [errors, setErrors] = useState(emptyErrors); const [isSubmitting, setIsSubmitting] = useState(false); const [submitError, setSubmitError] = useState('')
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); if (isSubmitting) return; const nextErrors = validateFinancialAnalysisForm(formState); setErrors(nextErrors); setSubmitError(''); if (hasFormErrors(nextErrors)) return; setIsSubmitting(true); clearResult(); try { setResult(await analyzeFinancialSituation()); navigate('/analysis/result') } catch { setSubmitError('Não foi possível concluir a análise agora. Tente novamente.'); setIsSubmitting(false) } }
  return <div className="analysis-page"><header className="page-heading"><span className="eyebrow">Análise financeira</span><h1>Vamos conhecer melhor a sua renda</h1><p>Preencha as informações para demonstrar o fluxo do protótipo. Nenhum dado será enviado a uma API nesta etapa.</p></header><form className="analysis-form" onSubmit={submit} noValidate><IncomeHistorySection items={formState.incomeHistory} errors={errors.incomeHistory} onAdd={addIncomeMonth} onChange={updateIncomeMonth} onRemove={removeIncomeMonth} /><SavingFrequencyField value={formState.savingFrequency} error={errors.savingFrequency} onChange={(savingFrequency) => setFormState((current) => ({ ...current, savingFrequency }))} /><TransactionsSection items={formState.transactions} errors={errors.transactions} onAdd={addTransaction} onChange={updateTransaction} onRemove={removeTransaction} /><MonthlyCommitmentsSection />{submitError && <div className="submit-error" role="alert">{submitError}</div>}<div className="form-footer"><p aria-live="polite">{isSubmitting ? 'Sua análise está sendo processada.' : 'O resultado será mantido somente durante esta sessão.'}</p><div><ButtonLink to="/" variant="secondary">Voltar ao início</ButtonLink><button className="button button-primary" type="submit" disabled={isSubmitting}>{isSubmitting ? 'Analisando...' : 'Analisar situação financeira'}</button></div></div></form></div>
}
